...
const sinirli = (req, res, next) => {
  ...
  if (token) {
    jwt.verify(token, JWT_SECRET, (error, decodeJwT) => {
      if (error) {
        next({ status: 401, message: "Token gecersizdir" });
      } else {
        req.userInfo = decodeJwT;
        next();
      }
    });
  } else {
...
const sadece = (role_name) => (req, res, next) => {
  if (req.headers.authorization && req.userInfo.role_name === role_name) next();
  else {
...
const usernameVarmi = (req, res, next) => {
  UserModels.goreBul({ username: req.body.username.trim() })
    .then((response) => {
      response.length > 0
        ? next()
...
const rolAdiGecerlimi = async (req, res, next) => {
  ...
  if (!role_name || role_name.trim() === "") {
    (req.body.role_name = "student"), next();
  } else {
